...
export function renderSessionLine(ctx) {
    ...
    if (display?.showUsage !== false && ctx.usageData && !shouldHideUsage(ctx.stdin)) {
        ...
        const renderBalance = (text) => `${getBalanceColor(text, colors)}${text}${RESET}`;
            ...
            if (effectiveUsage >= usageThreshold) {
                ...
                if (usageCompact) {
                    const fiveHourPart = fiveHour !== null
                        ? formatCompactWindowPart('5h', fiveHour, ctx.usageData.fiveHourResetAt, timeFormat, colors, usageValueMode, stale)
                        : null;
                    const sevenDayThreshold = display?.sevenDayThreshold ?? 80;
                    const sevenDayPart = (sevenDay !== null && (fiveHour === null || sevenDay >= sevenDayThreshold))
                        ? formatCompactWindowPart('7d', sevenDay, ctx.usageData.sevenDayResetAt, timeFormat, colors, usageValueMode, stale)
                        : null;
...
function formatCompactWindowPart(windowLabel, percent, resetAt, timeFormat, colors, usageValueMode = 'percent', stale = false) {
...
function formatUsageWindowPart({ label: windowLabel, percent, resetAt, colors, usageBarEnabled, barWidth, timeFormat = 'relative', showResetLabel, forceLabel = false, usageValueMode = 'percent', stale = false, }) {
    const usageDisplay = formatUsagePercent(percent, colors, usageValueMode, stale);
    const reset = formatResetTime(resetAt, timeFormat);
    const styledLabel = label(windowLabel, colors);
