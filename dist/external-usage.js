 ...
 */
function resolveAutoSnapshotPath(stdin) {
...
export function getUsageFromExternalSnapshot(config, now = Date.now(), stdin) {
...
export function triggerRefreshIfStale(config, now = Date.now()) {
    if (refreshScheduled)
        return;
    if (!config.display.externalUsagePath && !config.display.externalUsageFreshnessMs)
        return;
    const statePath = path.join(os.tmpdir(), 'claude-hud-last-refresh');
    try {
        const last = parseInt(fs.readFileSync(statePath, 'utf8'), 10);
        if (!Number.isNaN(last) && now - last < REFRESH_COOLDOWN_MS)
            return;
    }
    catch { }
